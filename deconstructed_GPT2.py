...
class Rep:
    ...
    def __init__(self, *args, **kwargs):
        ...
        setattr(self, '_device', 'gpu') # CUDA processing
...
class DeconstructedGPT2(nn.Module):
    ...
    def __init__(self, model_name, dataset=None,
        ...
        if model is None:
            self._model = get_model(model_name)#m
            self._device = 'gpu'#m
        else:
            self._model = model
            self._device = device
        ...
        self._num_of_layers = len(self._model.config.num_hidden_layers) #m
    ...
    def _emb(self, v, **kwargs):
        ...
        inputs_embeds = self._model.get_input_embeddings(result)
        position_ids = torch.arange(0, result.size()[-1],
                                    dtype=torch.long,
                                    device=self._device)
        position_embeds = self._model.wpe(position_ids)
        # result = inputs_embeds + position_embeds #Mamba doesn't have positionial embedding
        result = input_embeds #check how mamba model deals with positional embedding
